      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 pb-20 relative z-10">
        ...
        `}>
          ...
          <div className="space-y-6">
            <p className={`text-xl md:text-2xl text-center leading-relaxed ${isDarkMode ? 'text-dark-100' : 'text-dark-700'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
              <div className="bg-gradient-to-br from-primary-400/15 to-primary-500/5 p-6 rounded-xl border border-primary-500/30 text-center hover:-translate-y-2 transition-transform">
                <h4 className={`font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>📧 البريد الإلكتروني</h4>
                <p className="text-primary-600 font-semibold text-sm">braaomer98@gmail.com</p>
              </div>
              <div className="bg-gradient-to-br from-primary-400/15 to-primary-500/5 p-6 rounded-xl border border-primary-500/30 text-center hover:-translate-y-2 transition-transform">
                <h4 className={`font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>📱 الهاتف</h4>
                <p className="text-primary-600 font-semibold text-sm">+249926699368</p>
              </div>
              <div className="bg-gradient-to-br from-primary-400/15 to-primary-500/5 p-6 rounded-xl border border-primary-500/30 text-center hover:-translate-y-2 transition-transform">
                <h4 className={`font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>📍 الموقع</h4>
                <p className="text-primary-600 font-semibold text-sm">الخرطوم، السودان</p>
              </div>
              <div className="bg-gradient-to-br from-primary-400/15 to-primary-500/5 p-6 rounded-xl border border-primary-500/30 text-center hover:-translate-y-2 transition-transform">
                <h4 className={`font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>🎂 العمر</h4>
                <p className="text-primary-600 font-semibold text-sm">27 سنة</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {skills.map((skill, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className={`font-semibold text-lg ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>
                    {skill.name}
                  </span>
                  <span className={`text-sm font-semibold ${isDarkMode ? 'text-dark-200' : 'text-dark-600'}`}>
                    {skill.level}%
                  </span>
                </div>
                <div className={`w-full h-3 rounded-full overflow-hidden ${isDarkMode ? 'bg-dark-700' : 'bg-primary-200'}`}>
                  <div
        ...
        <div className="mb-12">
          <div className="text-center mb-12">
            <h3 className={`text-4xl font-black mb-4 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>
              <span className="bg-gradient-to-r from-primary-500 to-primary-700 bg-clip-text text-transparent">
                الخبرات العملية
              </span>
            </h3>
            <p className={`text-lg ${isDarkMode ? 'text-dark-100' : 'text-dark-700'}`}>
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="hidden md:block absolute right-1/2 transform translate-x-1/2 w-1 h-full bg-gradient-to-b from-primary-500 via-primary-600 to-primary-700 rounded-full" />
              ...
              {experiences.map((exp, index) => (
                <div key={index} className="relative">
                  ...
                  <div className={`hidden md:flex items-center ${index % 2 === 0 ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`w-5/12 ${index % 2 === 0 ? 'pr-8' : 'pl-8'}`}>
                      ...
                      `}>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="text-sm font-semibold text-primary-600 bg-primary-500/10 px-3 py-1 rounded-full border border-primary-500/30">
                            {exp.period}
                          </span>
                        </div>
                        ...
                        <p className={isDarkMode ? 'text-dark-200' : 'text-dark-600'}>
                          {exp.description}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Timeline dot */}
                    <div className="w-12 h-12 bg-white border-4 border-primary-600 rounded-full flex items-center justify-center z-10 shadow-lg">
                      <div className="w-3 h-3 bg-gradient-to-r from-primary-500 to-primary-600 rounded-full" />
                    </div>
                  ...
                  <div className="md:hidden pr-12 relative">
                    ...
                    `}>
                      ...
                      <h4 className={`text-lg font-black mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>
                        {exp.title}
                      </h4>
                      <p className="text-primary-600 font-semibold mb-2 text-sm">{exp.company}</p>
                    ...
                    {/* Mobile timeline dot */}
                    <div className="absolute right-3 top-6 w-6 h-6 bg-white border-3 border-primary-600 rounded-full flex items-center justify-center">
                      <div className="w-1.5 h-1.5 bg-gradient-to-r from-primary-500 to-primary-600 rounded-full" />
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className={`
              ...
              border border-primary-500/30 shadow-xl
              hover:-translate-y-3 hover:scale-105 transition-all duration-300
              ${isDarkMode ? 'bg-dark-800/50' : 'bg-white/50'}
            `}>
              <div className="text-5xl mb-4">🏆</div>
              <h4 className={`text-xl font-black mb-2 ${isDarkMode ? 'text-white' : 'text-dark-950'}`}>
                بكالريوس نظم المعلومات
              </h4>
              <p className="text-primary-600 font-semibold">النيلين - 2024</p>
            </div>
            ...
            `}>
              ...
              <p className="text-primary-600 font-semibold">SoloLerin</p>
            </div>
          </div>
        </div>
      </div>
